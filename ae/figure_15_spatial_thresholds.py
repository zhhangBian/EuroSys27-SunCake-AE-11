...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Figure 15: spatial threshold sensitivity."
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("ae/results/figure_15_spatial_thresholds"),
    )
    parser.add_argument(
    ...
    parser.add_argument("--port-base", type=int, default=8555)
    ...
    parser.add_argument("--critical-ratio", type=float, default=0.75)
    parser.add_argument("--gpu-usage-highs", default="0.05,0.06,0.08")
    parser.add_argument("--gpu-usage-low", type=float, default=0.02)
    parser.add_argument("--adjustment-step", type=float, default=0.05)
...
def main() -> int:
    ...
    ratio, low, step = args.critical_ratio, args.gpu_usage_low, args.adjustment_step
    for high in parse_csv_floats(args.gpu_usage_highs):
        if not 0 <= low < high <= 1:
            raise SystemExit("Watermarks must satisfy 0 <= low < high <= 1")
        name = (f"r{ratio:g}_hi{high:g}_lo{low:g}_step{step:g}").replace(".", "_")
        modes.append(
            ModeSpec(
                name=name,
                ...
                extra_env=tuple(
                    base_env
                    + [
                        ("VLLM_AGENT_IMPORTANT_AGENT_RATIO", str(ratio)),
                        ("VLLM_AGENT_GPU_USAGE_HIGH_WATERMARK", str(high)),
                        ("VLLM_AGENT_GPU_USAGE_LOW_WATERMARK", str(low)),
                        ("VLLM_AGENT_RESERVE_ADJUSTMENT_STEP", str(step)),
                    ]
                ),
            )
        )
    ...
    write_markdown_cases(
        run_root / "report.md", "Figure 15: Spatial Thresholds", cases, blocked
    )
    print(run_root)
