...
class ParrotServeCore:
    ...
    async def submit_common_inference(self, payload: Dict) -> Dict:
        """Submit a plain prompt-based inference request.

        This is a thin wrapper over the existing semantic-call path. It treats the
        full prompt as one constant fill node plus a single output placeholder.
        """

        if "prompt" not in payload:
            raise ParrotCoreUserError("Missing field 'prompt' in common inference request.")
        if "model" not in payload:
            raise ParrotCoreUserError("Missing field 'model' in common inference request.")
        ...
        output_criteria = payload.get("output_criteria", "latency")
        ...
        try:
            submit_response = self.submit_semantic_call(
                {
                    "session_id": session_id,
                    "template": f"{prompt}{{{{output}}}}",
                    "parameters": [
                        {
                            "name": "output",
                            "is_output": True,
            ...
            for param in submit_response["param_info"]:
                if param["is_output"]:
                    var_id = param["var_id"]
                    break

            if var_id is None:
                raise ParrotCoreInternalError(
                    "Common inference request did not create an output variable."
                )

            result = await self.get_semantic_variable(
                var_id,
                {
                    "session_id": session_id,
            ...
            try:
                ...
                logger.warning(
                    "Failed to remove common inference session %s cleanly.",
                    session_id,
                )
