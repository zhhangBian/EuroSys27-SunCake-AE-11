...
def hf_weights_loader(model_name: str) -> Iterator[Tuple[str, torch.Tensor]]:
    ...
    hf_safetensors_files = glob.glob(os.path.join(hf_folder, "*.safetensors"))
    ...
    for safetensors_file in hf_safetensors_files:
        state = load_safetensors_file(safetensors_file, device="cpu")
        for name, param in state.items():
            yield name, param
        del state
        torch.cuda.empty_cache()
