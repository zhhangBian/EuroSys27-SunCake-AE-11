...
class LlamaAttention(nn.Module):
    ...
    def __init__(
        ...
        qkv_bias = bool(getattr(config, "attention_bias", False)) or (
            getattr(config, "model_type", "") == "qwen2"
        )
        self.q_size = self.num_heads * self.head_dim
        self.kv_size = self.num_key_value_heads * self.head_dim
        self.qkv_proj = nn.Linear(
            self.hidden_size, self.q_size + 2 * self.kv_size, bias=qkv_bias
        )
    ...
    def load_weights(self, model_name_or_path: str):
        ...
        for weight_name, weight_value in hf_weights_loader(model_name_or_path):
            ...
            for stride_id, qkv_proj_name in enumerate(["q_proj", "k_proj", "v_proj"]):
                ...
                q_size = self.model.layers[0].self_attn.q_size
                kv_size = self.model.layers[0].self_attn.kv_size
                shard_sizes = [q_size, kv_size, kv_size]
                shard_start = sum(shard_sizes[:stride_id])
                shard_end = shard_start + shard_sizes[stride_id]

                param_slice = param.data[shard_start:shard_end]
