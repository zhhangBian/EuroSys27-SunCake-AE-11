...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        temperature = torch.tensor(
            [sf.temperature for sf in sampling_config],
            dtype=torch.float,
            device=logits.device,
        )
        greedy_mask = temperature <= 0
        non_greedy_mask = ~greedy_mask

        if greedy_mask.any():
            sampled_ids[greedy_mask] = logits[greedy_mask].argmax(dim=-1)

        if not non_greedy_mask.any():
            return sampled_ids

        logits = logits[non_greedy_mask]
        active_sampling_config = [
            config
            for config, enabled in zip(sampling_config, non_greedy_mask.tolist())
            if enabled
        ]

        # Applying temperature scaling
        active_temperature = temperature[non_greedy_mask]
        if any(t != 1.0 for t in active_temperature.tolist()):
            logits = logits / active_temperature.unsqueeze(-1)

        # Applying top_p
        top_ps = [sf.top_p for sf in active_sampling_config]
        ...
        sampled_ids[non_greedy_mask] = torch.multinomial(
