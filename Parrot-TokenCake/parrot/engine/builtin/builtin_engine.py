...
class BuiltinEngine(LLMEngine):
    ...
    def __init__(self, engine_config: Dict, connect_to_core: bool = True):
        ...
        tokens_capacity = (
            builtin_config.num_kv_cache_blocks * builtin_config.block_size
        )
        self.engine_config.tokens_capacity = tokens_capacity
        if scheduler_config.max_total_tokens > tokens_capacity:
            logger.warning(
                f"Clamp scheduler max_total_tokens from "
                f"{scheduler_config.max_total_tokens} to KV cache capacity "
                f"{tokens_capacity}."
            )
            scheduler_config.max_total_tokens = tokens_capacity
