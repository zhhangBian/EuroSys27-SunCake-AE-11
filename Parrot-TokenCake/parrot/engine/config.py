...
@dataclass
class BuiltinConfig:
    num_kv_cache_blocks: Optional[int] = None
    attn_func: Union[str, "AttnFunc"] = "xformers_with_buffer"
    ...
    def __post_init__(self):
        if self.gpu_memory_utilization is not None:
            if not (0.0 < self.gpu_memory_utilization <= 1.0):
                raise ValueError(
                    "GPU memory utilization must be in (0.0, 1.0]. Got "
                    f"{self.gpu_memory_utilization}."
                )
        elif self.num_kv_cache_blocks is None or self.num_kv_cache_blocks <= 0:
            raise ValueError(
                "num_kv_cache_blocks must be a positive integer when "
                "gpu_memory_utilization is not set."
            )
        ...
        if not hasattr(self, "_get_attn_func"):
            from .builtin import attn_func  # noqa: F401
