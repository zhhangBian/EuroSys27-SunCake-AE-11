...
class AsyncLLM(EngineClient):
    ...
    async def get_mcp_debug_state(self) -> dict:
        return await self.engine_core.get_mcp_debug_state_async()
    ...
    async def add_request_to_offload(
        self,
        request_id: str,
        ...
        logger.info(f"[AsyncLLM] Scheduling KV Cache offload for request {request_id}, Estimated time: {estimated_time}")
        await self.engine_core.add_request_to_offload_async(
            request_id, estimated_time, request_type, started_at)

    async def mark_request_finished(
        self,
        request_id: str,
        request_type: Optional[str] = None,
    ) -> None:
        logger.info(f"[AsyncLLM] Marking request {request_id} as finished, triggering KV cache upload")
        await self.engine_core.mark_request_finished_async(
