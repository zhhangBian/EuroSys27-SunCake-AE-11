...
def _get_cpu_swap_memory_bytes(vllm_config: VllmConfig) -> int:
    cache_config = vllm_config.cache_config
    if not cache_config.enable_kvcache_cpu_offloading:
        return 0
    return cache_config.swap_space_bytes
...
class EngineCore:
    ...
    def _initialize_kv_caches(
        ...
        available_cpu_swap_memory = _get_cpu_swap_memory_bytes(vllm_config)
    ...
    def get_mcp_debug_state(self) -> dict[str, Any]:
        if hasattr(self.scheduler, "mcp_manager"):
            return self.scheduler.mcp_manager.get_debug_state()
        return {}
    ...
    def add_request_to_offload(
        ...
        logger.info(f"[EngineCore] received offload request: {request_id}, {estimated_time}")
        logger.info(f"[EngineCore] Added {request_id} to MCPFunctionManager offload queue")
        self.scheduler.mcp_manager.add_request_to_offload(
            request_id, estimated_time, request_type, started_at=started_at)
        return True

    def mark_request_finished(self,
                              request_id: str,
                              request_type: Optional[str] = None) -> bool:
        if hasattr(self.scheduler, 'mcp_manager'):
            self.scheduler: V1Scheduler
            logger.info("[EngineCore] Tool finished for request %s", request_id)
            self.scheduler.mcp_manager.mark_request_finished(
                request_id, request_type=request_type)
            return True
        else:
            logger.warning("[EngineCore] Scheduler does not have mcp_manager, cannot mark request as finished")
            return False
...
class EngineCoreProc(EngineCore):
    ...
    def _handle_client_request(self, request_type: EngineCoreRequestType,
        ...
        if request_type == EngineCoreRequestType.ADD:
        ...
        elif request_type == EngineCoreRequestType.MCP_OFFLOAD:
            if len(request) == 2:
                request_id, estimated_time = request
                mcp_request_type = None
                started_at = None
            elif len(request) == 3:
                request_id, estimated_time, mcp_request_type = request
                started_at = None
            else:
                (request_id, estimated_time, mcp_request_type,
                 started_at) = request
            logger.info(f"[EngineCore] Received MCP_OFFLOAD request for {request_id}")
            ...
            if hasattr(self.scheduler, 'mcp_manager'):
                ...
                logger.info(f"[EngineCore] Added {request_id} to offload queue")
            else:
                logger.warning(f"[EngineCore] Scheduler does not have mcp_manager, cannot process MCP_OFFLOAD request for {request_id}")

            self.output_queue.put_nowait(EngineCoreOutputs())
        elif request_type == EngineCoreRequestType.MCP_FINISHED:
            if isinstance(request, (list, tuple)):
                request_id, mcp_request_type = request
            else:
                request_id = request
                mcp_request_type = None
            logger.info(f"[EngineCore] Received MCP_FINISHED request for {request_id}")

            if hasattr(self.scheduler, 'mcp_manager'):
                self.scheduler.mcp_manager.mark_request_finished(
                    request_id, request_type=mcp_request_type)
                logger.info("[EngineCore] Tool finished for request %s", request_id)
            else:
                logger.warning(f"[EngineCore] Scheduler does not have mcp_manager, cannot process MCP_FINISHED request for {request_id}")
