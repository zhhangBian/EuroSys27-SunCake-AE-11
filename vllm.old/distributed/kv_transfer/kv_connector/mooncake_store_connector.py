...
class MooncakeStoreConnector(KVConnectorBase):
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            logger.info(
                ...
                json.dumps({
                    "op": "send",
                    "tokens": int(slen),
                    "layers": int(end_layer - start_layer),
                    "tp_rank": int(self.local_tp_rank),
                    "elapsed_ms":
                    (time.perf_counter() - send_start_time) * 1000.0,
                }, sort_keys=True),
            )
    ...
    def recv_kv_caches_and_hidden_states(
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            logger.info(
                ...
                json.dumps({
                    "op": "recv",
                    "hit": True,
                    "tokens": int(num_computed_tokens),
                    "layers": int(end_layer - start_layer),
                    "tp_rank": int(self.local_tp_rank),
                }, sort_keys=True),
            )
        ...
        logger.info(
            ...
            json.dumps({
                "op": "recv_batch",
                "bypass_model_exec": bool(bypass_model_exec),
                "full_hits": int(full_hits),
                "misses": int(misses),
                "seq_count": int(len(seq_lens)),
                "elapsed_ms": (time.perf_counter() - recv_start_time) *
                1000.0,
            }, sort_keys=True),
        )
