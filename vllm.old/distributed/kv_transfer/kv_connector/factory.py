...
class KVConnectorFactory:
    ...
    def create_connector_v1(
        ...
        if connector_name == "MooncakeStoreConnector":
            from vllm.distributed.kv_transfer.kv_connector.v1.mooncake_store_connector import (
                MooncakeStoreConnectorV1)

            logger.info("Creating native V1 connector for %s", connector_name)
            return MooncakeStoreConnectorV1(config, role)

        if issubclass(connector_cls, KVConnectorBase_V1):
        ...
        raise TypeError(
            f"Connector {connector_name} does not implement the V1 interface.")
