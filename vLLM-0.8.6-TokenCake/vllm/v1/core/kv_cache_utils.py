...
class KVCacheBlock:
    ...
    block_in_cpu: bool = False
    related_block: Optional["KVCacheBlock"] = None
    ...
    def __repr__(self) -> str:
        ...
        related_block_id = (self.related_block.block_id
                            if self.related_block is not None else None)
        return (f"KVCacheBlock(block_id={self.block_id}, "
                f"ref_cnt={self.ref_cnt}, "
                f"cpu_offloading_block={related_block_id}, "
...
class FreeKVCacheBlockQueue:
    ...
    def contains(self, block: KVCacheBlock) -> bool:
        return block.block_id in self._free_block_ids
    ...
    def remove(self, block: KVCacheBlock) -> None:
        ...
        """
        if block.block_id not in self._free_block_ids:
            raise ValueError(
                f"Block {block.block_id} is not in the free block queue")
        ...
        if block is self.free_list_head:
            # Update the head if the block is the head.
            self.free_list_head = block.next_free_block
        if block is self.free_list_tail:
    ...
    def insert_before(self, block: KVCacheBlock,
                      next_block: KVCacheBlock) -> None:
        if self.contains(block):
            return
        if not self.contains(next_block):
            raise ValueError(
                f"Block {next_block.block_id} is not in the free block queue")

        prev_block = next_block.prev_free_block
    ...
    def insert_after(self, block: KVCacheBlock,
                     prev_block: KVCacheBlock) -> None:
        if self.contains(block):
            return
        if not self.contains(prev_block):
            raise ValueError(
                f"Block {prev_block.block_id} is not in the free block queue")
    ...
    def get_all_free_blocks(self) -> list[KVCacheBlock]:
        ...
        for _ in range(len(self._free_block_ids)):
            if curr_block is None:
                raise RuntimeError(
                    "Free block queue ended before num_free_blocks")
            ret.append(curr_block)
            curr_block = curr_block.next_free_block
        if curr_block is not None:
            raise RuntimeError(
                "Free block queue contains a cycle or exceeds num_free_blocks")
        return ret


class FreeBlockQueueWithBuffer(FreeKVCacheBlockQueue):
    def __init__(self, blocks: list[KVCacheBlock]) -> None:
        super().__init__(blocks)
        self.buffer: dict[int, KVCacheBlock] = {}

    def contains(self, block: KVCacheBlock) -> bool:
        return block.block_id in self.buffer or super().contains(block)

    def append(self, block: KVCacheBlock) -> None:
        if self.contains(block):
            return
        self.buffer[block.block_id] = block
        self.num_free_blocks += 1

    def remove(self, block: KVCacheBlock) -> None:
        if block.block_id in self.buffer:
            self.buffer.pop(block.block_id)
            self.num_free_blocks -= 1
            return
        super().remove(block)
    ...
    def popleft(self) -> KVCacheBlock:
        ...
        self.num_free_blocks -= 1
        block_id = next(iter(self.buffer))
        return self.buffer.pop(block_id)

    def get_all_free_blocks(self) -> list[KVCacheBlock]:
        return list(self.buffer.values()) + super().get_all_free_blocks()
