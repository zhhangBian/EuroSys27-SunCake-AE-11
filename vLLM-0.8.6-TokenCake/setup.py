...
if (envs.VLLM_USE_PRECOMPILED
        or envs.VLLM_TEST_USE_PRECOMPILED_NIGHTLY_WHEEL):
    raise RuntimeError(
        "SunCake requires its custom native KV-cache transfer kernels to be "
        "built from this source tree. Upstream precompiled vLLM wheels cannot "
        "replace them. Unset VLLM_USE_PRECOMPILED, "
        "VLLM_PRECOMPILED_WHEEL_LOCATION, and "
        "VLLM_TEST_USE_PRECOMPILED_NIGHTLY_WHEEL before building. "
        "VLLM_USE_PRECOMPILED=0 also enables the upstream precompiled path.")
...
def get_vllm_version() -> str:
    version = load_module_from_path(
        '_vllm_build_version', ROOT_DIR / 'vllm' / '_version.py').__version__
